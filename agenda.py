...
def adicionar_contato(nome, telefone, endereço, CPF):
    conexao = criar_conexao()
    cursor = conexao.cursor()
    cursor.execute("INSERT INTO contatos (nome, telefone, endereço, CPF) VALUES (?, ?, ?, ?)", (nome, telefone, endereço, CPF))
    conexao.commit()
    conexao.close()
...
def listar_contatos():
    ...
    if contatos:
        for contato in contatos:
            print(f"ID: {contato[0]}, Nome: {contato[1]}, Telefone: {contato[2]}, Endereço: {contato[3]}, CPF: {contato[4]}")
    else:
...
def buscar_contato_por_nome(nome):
    ...
    if resultado:
        print(f"Nome: {resultado[1]}, Telefone: {resultado[2]}, Endereço: {resultado[3]}, CPF: {resultado[4]}")
    else:
...
def menu():
    ...
    while True:
        ...
        if opcao == "1":
            ...
            CPF = input("CPF: ")
            adicionar_contato(nome, telefone, endereço, CPF)
        elif opcao == "2":
            listar_contatos()
        elif opcao == "3":
